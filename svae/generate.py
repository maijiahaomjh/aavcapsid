...
def cluster_latent_space(
    ...
    """
    对 Latent Space (隐空间) 进行聚类分析。
    
    目的: 识别 Latent Space 中的高活性区域 (Cluster)。
    
    参数:
        preds_df: 包含 Latent 坐标 (z0, z1, ...) 和真实值/预测值 (y_true, y_pred) 的 DataFrame。
        assay: 实验名称。
        clustering_method: 聚类方法 ('kmeans' 或 'dbscan')。
        cluster_with_assay_vals: 是否将活性值 (y) 也作为聚类特征之一 (不仅是用 z)。
                                 如果为 True，会将 y 进行缩放后拼接到 z 上一起聚类。
        y_scale_adjustment: y 值在聚类时的权重缩放因子。
        
    返回:
        preds_df: 添加了 'cluster_label' 和 'cluster_assay_mean' 列的 DataFrame。
        dict: 包含聚类中心和每个簇的平均活性值。
    """
    ...
    # 缩放标签 Y，使其与 Z 在同一量级，以便在聚类时同时考虑位置和活性
    scaled_labels = (
    ...
    # 计算每个 Cluster 的平均活性 (log2enr)，并添加到 preds_df
...
def GMM_on_clusters(
    ...
    """
    对每个聚类拟合一个高斯分布 (Gaussian)，并从中采样生成新序列，然后用 Regressor 预测其活性。
    
    目的: 验证聚类的质量，并探索聚类中心附近的序列活性。
    
    参数:
        model: SVAE 模型 (用于解码和预测)。
        latent_df: 包含聚类信息的 DataFrame。
        cluster_centers: 聚类中心坐标。
        test_samples_per_cluster: 每个聚类采样的样本数。
        var_scale: 采样方差的缩放因子 (控制探索范围)。
    """
...
def generate_without_sampling(
    ...
    生成策略:
    1. 选出活性最高的 Cluster (Primary Cluster)。
    2. 在该 Cluster 内再进行一轮聚类 (Sub-clustering)。
    3. 对每个 Sub-cluster，计算其氨基酸频率矩阵 (Logo)。
    4. 筛选出 Logo 中频率高于阈值的氨基酸。
    5. 对这些高频氨基酸进行排列组合 (Combinatorial)，生成新序列。
    
    优点: 这种方法比直接随机采样更稳定，能生成符合 Cluster 模式的高质量序列。
    """
...
def generate_sequences(
    ...
    流程:
    1. 调用 cluster_latent_space 对训练集/验证集的 Latent 空间进行聚类。
    2. 调用 generate_without_sampling 基于聚类结果生成新序列。
    
    返回:
        dict: 按 Cluster 组织的生成序列字典。
        list: 所有生成的唯一序列列表。
    """
