...
数据处理工具模块
================
此模块包含用于处理生物序列数据的工具函数，包括：
1. 序列编码 (One-hot Encoding)
2. 数据清洗与过滤 (prep_data)
3. TensorFlow 数据集生成 (make_tf_batches)
"""
...
def prep_data(
    ...
    """
    准备模型训练数据：清洗、过滤、编码并划分测试集。
    
    参数：
        df (pd.DataFrame)：原始数据框。
        target_assay_col (str)：目标值列名 (例如 'log2_enrichment')。
        cv_col (str, optional)：变异系数列名 (用于加权 Loss)，可选。
        RPM_col (str, optional)：Reads Per Million 列名，用于过滤低丰度序列。
        spec_col (str, optional)：特异性列名，用于过滤。
        target_threshold (float)：目标值阈值，低于此值的样本会被丢弃。
        RPM_threshold (float)：RPM 阈值。
        spec_threshold (float)：特异性阈值。
        test_size (float)：测试集比例 (0.0 - 1.0)。
        alphabet (np.array)：氨基酸字母表。
        AA_colname (str)：包含氨基酸序列的列名。
        random_seed (int)：随机种子，保证结果可复现。
        
    返回：
        tuple：(train_df, test_df)
            - train_df：训练+验证数据 (包含 One-hot 编码列)。
            - test_df：独立测试数据。
    """
